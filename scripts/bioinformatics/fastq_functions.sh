...
merge_fastq_files() {
    if [[ "$#" -ne 2 ]]; then
        echo "Usage: merge_fastq_files <input_directory> <output_directory>" >&2
        return 1
    fi
    ...
    for i in $(find "$input_dir" -type f -name "*.fastq.gz" | while read -r F; do basename "$F" | rev | cut -c 22- | rev; done | sort | uniq)
    do
        echo "Merging R1"
